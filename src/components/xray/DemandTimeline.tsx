...
function recencyLabel(days: number): { text: string; color: string } {
  if (days <= 7) return { text: 'This week', color: 'text-emerald-600' };
  if (days <= 30) return { text: 'This month', color: 'text-emerald-600' };
  if (days <= 90) return { text: `${Math.round(days / 7)} weeks ago`, color: 'text-amber-600' };
  if (days <= 365) return { text: `${Math.round(days / 30)} months ago`, color: 'text-amber-600' };
  return { text: `${(days / 365).toFixed(1)} years ago`, color: 'text-red-600' };
}
...
const TrendIcon = ({ trend }: { trend: 'up' | 'down' | 'neutral' }) => {
  ...
  return (
    <svg className="h-4 w-4 text-[#94A3B8]" viewBox="0 0 20 20" fill="currentColor">
      <path
        fillRule="evenodd"
    ...
    <div className="rounded-[12px] border border-[#E2E8F0] bg-[#FFFFFF] p-5">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <div className="space-y-3">
          {/* Downloads */}
          <div className="rounded-[12px] border border-[#E2E8F0] bg-[#F8FAFC] px-5 py-4 transition-colors hover:border-[#CBD5E1]">
          ...
          <div className="rounded-[12px] border border-[#E2E8F0] bg-[#F8FAFC] px-5 py-4 transition-colors hover:border-[#CBD5E1]">
            ...
            <div className="flex items-baseline gap-1.5">
              ...
              <svg
                className="ml-auto h-4 w-4 self-center text-red-600"
                viewBox="0 0 20 20"
                fill="currentColor"
              >
            ...
            <p className="mt-1 text-xs text-[#94A3B8]">Community engagement</p>
          </div>
          ...
          <div className="rounded-[12px] border border-[#E2E8F0] bg-[#F8FAFC] px-5 py-4 transition-colors hover:border-[#CBD5E1]">
            ...
            <div className="flex items-baseline gap-1.5">
              ...
              <span className="text-sm font-medium text-[#94A3B8]">of total</span>
            </div>
            <p className="mt-1 text-xs text-[#94A3B8]">
        ...
        <div className="space-y-3">
          {/* Lab tier badge */}
          <div className="rounded-[12px] border border-[#E2E8F0] bg-[#F8FAFC] px-5 py-4">
            <p className="mb-2 text-xs font-medium uppercase tracking-wide text-[#475569]">
              Lab / Publisher
            </p>
            <p className="mb-2 text-sm font-medium text-[#0F172A]">{lab_name}</p>
            <span
